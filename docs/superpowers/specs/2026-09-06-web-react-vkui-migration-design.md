...
- **DI-ретрофит (2026-09-06).** Логика сессии и авторизации, изначально жившая в React-контексте (`SessionProvider`) и компонентах форм, перенесена в хуки `model/` (`useSessionController`, `useLoginForm`, `useRegisterForm`, `useAuthRedirect`, `useLogout`), а побочные эффекты (`ApiClient`, `UnauthorizedBus`, `ColorSchemeStore`) — за порты `entities/session` (`SESSION_GATEWAY`), `features/auth` (`AUTH_GATEWAY`) и `shared/api`/`shared/lib`, забинженные в `app/composition/container.ts` через контейнер InversifyJS (`shared/di`). Подробности — раздел «DI (InversifyJS)» в `README.md`.
