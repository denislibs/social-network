...
- Правило повторной заявки после отклонения: исходный инициатор может отправить заявку заново через 24 часа после `createdAt` (`RequestCooldown`); в спеке было решение «отклонение не мешает подписке», но не было явного правила про повторную заявку — принято по ходу задачи 3.
