...
- Загрузка как у VK: не спиннеры, а скелетоны той же формы, что контент, из VKUI `Skeleton` внутри тех же `Group`/`SimpleCell`; парный компонент `<Name>Skeleton` рядом с виджетом, `aria-busy="true"` на контейнере, показывается только при первой загрузке (`isPending`). `PanelSpinner` только как `Suspense`-fallback маршрутов и в `RequireAuth`.
...
- Без декораторов и без `reflect-metadata`: не включаем `experimentalDecorators`/`emitDecoratorMetadata` (TS 7 и Vite). Биндинги только явные: `bind(TOKEN).toConstantValue(x)`, `bind(TOKEN).toResolvedValue((a, b) => new Impl(a, b), [A_TOKEN, B_TOKEN])`. Контейнер приложения синглтонный по умолчанию (`defaultScope: 'Singleton'`); для объектов, которым нужен новый экземпляр на каждое обращение, явно указывать `.inTransientScope()`.
- В тестах: `renderHook(..., { wrapper })` с `DiProvider` над тестовым контейнером, где нужные токены забиндены на фейки (`toConstantValue`). Глобальный контейнер в тестах не трогаем. `vi.mock` разрешён для (a) сторонних модулей без DI-шва (VKUI, react-router), (b) `@/shared/api` в тестах самого клиента и (c) первостатейных хуков контекста, у которых по замыслу нет DI-шва (`useSession`), предпочтительно через wrapper-хелпер; для сервиса с токеном — никогда, там биндится фейк. Каждый метод шлюза либо передаёт в `unwrap` инжектированный `UnauthorizedBus`, либо явно указывает `silent401: true` — компилятор (`UnwrapOptions`) не даёт об этом забыть.
- `useService` резолвит зависимость во время рендера, поэтому биндинг `.inTransientScope()` отдаёт новый экземпляр на каждый рендер — никогда не используйте transient-скоуп для того, что хук кладёт в зависимости `effect`/`memo`.
